...
| Testing requirements | Implemented | `tests/*`, Playwright tests, benchmark script | Unit/integration/frontend/production-mode tests and benchmark tooling exist. Benchmark output separates external evaluator datasets from synthetic regression checks. |
...
| --- | --- | --- | --- |
| OCR CER/WER/accuracy | Implemented | `backend/benchmarks/metrics.py`, `backend/benchmarks/evaluate.py`, `backend/benchmarks/results.json` | Computed for synthetic regression and external datasets when reference text is available. |
| Field detection and extraction accuracy | Implemented | benchmark evaluator and metric tests | SROIE maps company/date/total to FinSight merchant/date/amount. CORD reports cleanly mapped total/subtotal/tax fields. FUNSD is a structure stress test, not receipt accuracy. |
| Categorization precision/recall/F1 | Implemented | `classification_metrics` helper | Macro and weighted F1 plus per-category metrics are generated for synthetic labels. External datasets do not invent category scores when labels are absent. |
...
| Frontend evaluator panel | Implemented | `#benchmark-metrics` in `frontend/index.html` | Collapsible panel prioritizes external SROIE metrics when available and keeps synthetic scores in a secondary internal-regression section. |
