...
<p align="center">
OCR bill analysis • anomaly detection • spending intelligence • AI investment assistance
</p>
...
Experience the full deployed FinSight platform here:

<p align="center">
  <a href="https://finsight-financial-intelligence-system-production.up.railway.app/#dashboard-view">
    https://finsight-financial-intelligence-system-production.up.railway.app/#dashboard-view
  </a>
</p>

The live deployment showcases the complete production-style workflow including:

- OCR-powered bill extraction
- AI-assisted transaction analysis
- Duplicate and anomaly detection
- Financial visualizations and analytics
- Real-time dashboard interaction
- Nova AI investment assistant
- Responsive fintech UI modules

Unlike half the “AI finance dashboards” floating around GitHub that are basically a chatbot duct-taped onto a Bootstrap template, FinSight actually implements an end-to-end intelligence pipeline with structured financial processing and benchmark-backed evaluation.
...
Use `--no-download` to verify missing-dataset behavior without network calls, or `--dataset-dir <path>` for local JSON/JSONL rows.

SROIE is the primary receipt field extraction benchmark; CORD v2 is treated as receipt OCR/layout robustness; FUNSD is treated as a document-structure stress test, not a receipt accuracy score.

Full external datasets are not run in CI or Railway production.
...
Attach a Railway Volume at `/data` for demo persistence.
...
Automated KPI metrics now separate external evaluator results from synthetic regression results.

Implemented calculations include:

* OCR CER/WER/accuracy
* Field detection rate
* Field accuracy by field
* Amount accuracy within INR 1
* Date parse rate
* Category precision/recall/F1 where labels exist
* Duplicate detection rate
* Anomaly recall
* Confidence calibration buckets
* Pipeline timing

Review-event correction rate, Nova groundedness/retrieval precision, chatbot relevance, and real savings validation are documented as deferred until review/chat logging or human labels exist.
